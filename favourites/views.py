from django.shortcuts import render
from django.http import Http404
from rest_framework import status
from rest_framework.views import APIView
from rest_framework.response import Response
from .models import Favourites
from .serializers import FavouritesSerializer
from wifi_wander_api.permissions import IsOwnerOrReadOnly

class FavouritesList(APIView):

    def get(self, request):
        favourites = Favourites.objects.all()
        serializer = FavouritesSerializer(
            favourites, many=True, context={'request': request}
        )
        return Response(serializer.data)
    
    def post(self, request):
        serializer = FavouritesSerializer(data=request.data, context={'request': request})
        if serializer.is_valid():
            serializer.save(user=request.user)
            return Response(serializer.data, status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
    

class FavouritesDetail(APIView):
    permission_classes = [IsOwnerOrReadOnly]

    def get_object(self, pk):
        try:
            favourites = Favourites.objects.get(pk=pk)
            self.check_object_permissions(self.request, favourites)
            return favourites
        except Favourites.DoesNotExist:
            raise Http404

    def get(self, request, pk):
        favourites = self.get_object(pk)
        serializer = FavouritesSerializer(favourites, context={'request': request})
        return Response(serializer.data)


    def put(self, request, pk):
        favourites = self.get_object(pk)
        serializer = FavouritesSerializer(
            favourites, data=request.data, context={'request': request}
        )
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
    
    def delete(self, request, pk):
        favourites = self.get_object(pk)
        favourites.delete()
        return Response({"msg": "Wifi Location Removed"}, status=status.HTTP_204_NO_CONTENT)
